...
enum location_type_t { kLocationTypeInvalid = 0, kLocationTypeStack = 1, kLocationTypeRegisters = 2 };
                                                   ...
                                                   const struct go_common_symaddrs_t *symaddrs)
{
    ...
    if (conn_intf.type == symaddrs->internal_syscallConn) {
        ...
        bpf_probe_read(&conn_intf, sizeof(conn_intf), conn_intf.ptr + symaddrs->syscallConn_conn_offset);
    }
...
static __inline int probe_return_tls_conn_write_core(struct pt_regs *ctx, uint64_t id, uint32_t tgid,
                                                     struct go_tls_conn_args *args, struct tgid_goid_t tgoid)
{
    struct go_tls_symaddrs_t *symaddrs = bpf_map_lookup_elem(&go_tls_symaddrs_map, &tgid);
    if (symaddrs == NULL) {
        return 0;
    ...
    // bpf_printk("probe_return_tls_conn_write_core retval1.ptr: %d, loctype:%d, conn:%d",retval1.ptr ,
    // symaddrs->Write_retval1_loc.type, args->conn_ptr); If function returns an error, then there's no data to trace.
    if (retval1.ptr != 0) {
        return 0;
    }
    ...
    if (conn_info) {
        ...
        conn_info->ssl = true;
        process_syscall_data_with_conn_info(ctx, &write_args, tgid_fd, kEgress, retval0, conn_info,
                                            nested_syscall_fd_ptr->syscall_len, true, true);
                                                    ...
                                                    struct go_tls_conn_args *args, struct tgid_goid_t tgoid)
{
    ...
    if (conn_info) {
        conn_info->ssl = true;
        process_syscall_data_with_conn_info(ctx, &read_args, tgid_fd, kIngress, retval0, conn_info,
                                            nested_syscall_fd_ptr->syscall_len, true, true);
