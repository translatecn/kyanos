...
static bool __always_inline report_conn_evt(void *ctx, struct conn_info_t *conn_info, enum conn_type_t type,
                                            uint64_t ts)
{
    int zero = 0;
...
static void __always_inline report_syscall_buf_without_data(void *ctx, uint32_t seq, struct conn_id_s_t *conn_id_s,
                                                            size_t len, enum step_t step, uint64_t ts,
                                                            uint32_t ts_delta, enum source_function_t source_fn)
{
...
static void __always_inline report_syscall_buf(void *ctx, uint32_t seq, struct conn_id_s_t *conn_id_s, size_t len,
                                               enum step_t step, uint64_t ts, uint32_t ts_delta, const char *buf,
                                               enum source_function_t source_fn, bool prepend_length_header,
...
static void __always_inline report_syscall_evt(void *ctx, uint32_t seq, struct conn_id_s_t *conn_id_s, uint32_t len,
                                               enum step_t step, struct data_args *args, bool prepend_length_header,
                                               uint32_t length_header)
{
    report_syscall_buf(ctx, seq, conn_id_s, len, step, args->start_ts, args->end_ts - args->start_ts, args->buf,
                       args->source_fn, prepend_length_header, length_header);
}

static void __always_inline report_ssl_buf(void *ctx, uint32_t seq, struct conn_id_s_t *conn_id_s, size_t len,
                                           enum step_t step, uint64_t ts, uint32_t ts_delta, const char *buf,
                                           enum source_function_t source_fn, uint32_t syscall_seq, uint32_t syscall_len)
{
    size_t _len = len < MAX_MSG_SIZE ? len : MAX_MSG_SIZE;
    if (_len == 0) {
        return;
...
static void __always_inline report_ssl_evt(void *ctx, uint32_t seq, struct conn_id_s_t *conn_id_s, uint32_t len,
                                           enum step_t step, struct data_args *args, uint32_t syscall_seq,
                                           uint32_t syscall_len)
{
    report_ssl_buf(ctx, seq, conn_id_s, len, step, args->start_ts, args->end_ts - args->start_ts, args->buf,
                   args->source_fn, syscall_seq, syscall_len);
}
static void __always_inline report_syscall_evt_vecs(void *ctx, uint32_t seq, struct conn_id_s_t *conn_id_s,
                                                    uint32_t total_size, enum step_t step, struct data_args *args,
                                                    bool prepend_length_header, uint32_t length_header)
{
    int bytes_sent = 0;
#pragma unroll
    for (int i = 0; i < LOOP_LIMIT && i < args->iovlen && bytes_sent < total_size; ++i) {
        ...
        report_syscall_buf(ctx, seq, conn_id_s, iov_size, step, args->start_ts, args->end_ts - args->start_ts,
                           iov_cpy.iov_base, args->source_fn, prepend_length_header, length_header);
        prepend_length_header = false;
        bytes_sent += iov_size;
        seq += iov_size;
    }
    if (bytes_sent < total_size) {
        report_syscall_buf_without_data(ctx, seq, conn_id_s, total_size - bytes_sent, step, args->start_ts,
                                        args->end_ts - args->start_ts, args->source_fn);
    }
}

static __inline uint64_t gen_tgid_fd(uint32_t tgid, int fd) { return ((uint64_t)tgid << 32) | (uint32_t)fd; }
static __always_inline void process_sendfile_with_conn_info(void *ctx, struct sendfile_args *args, uint64_t tgid_fd,
                                                            ...
                                                            struct conn_info_t *conn_info, int32_t syscall_len,
                                                            bool is_ssl, bool with_data)
{
    ...
    if (conn_info->protocol != kProtocolUnknown && (conn_info->no_trace <= traceable)) { //, bytes_count
        report_syscall_buf_without_data(ctx, seq, &conn_id_s, bytes_count, step, args->start_ts,
                                        args->end_ts - args->start_ts, kSyscallSendfile);
    }
}
                                                                ...
                                                                bool is_ssl, bool with_data)
{
    ...
    if ((conn_info->protocol == kProtocolUnset || conn_info->protocol == kProtocolUnknown) && with_data) {
        ...
        struct protocol_message_t protocol_message
            = infer_protocol(args->buf, bytes_count, bytes_count, conn_info, trace_protocol);
        if (before_infer != protocol_message.protocol) {
            ...
            if (conn_info->role == kRoleUnknown && protocol_message.type != kUnknown) {
                conn_info->role
                    = ((direct == kEgress) ^ (protocol_message.type == kResponse)) ? kRoleClient : kRoleServer;
            }
            inferred = true;
    ...
    if ((conn_info->protocol != kProtocolUnknown && (inferred || conn_info->no_trace <= traceable)) ||
        ...
        if (is_ssl) {
            ...
            report_ssl_evt(ctx, seq, &conn_id_s, bytes_count, step, args,
                           syscall_len < 0 ? 0 : (syscall_seq - syscall_len), syscall_len < 0 ? 0 : syscall_len);
