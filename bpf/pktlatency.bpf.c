...
static __always_inline bool skb_l2_check(u16 header) { return !header || header == (u16)~0U; }
static __always_inline bool skb_l4_check(u16 l4, u16 l3) { return l4 == 0xFFFF || l4 <= l3; }
    ...
    switch (_C(skc, skc_family)) {
        ...
        case AF_INET6:
            bpf_probe_read_kernel(
                (void *)(key->dip), sizeof(struct in6_addr),
                (const void *)__builtin_preserve_access_index(&((typeof((skc)))((skc)))->skc_v6_daddr.in6_u.u6_addr32));
            bpf_probe_read_kernel((void *)(key->sip), sizeof(struct in6_addr),
                                  (const void *)__builtin_preserve_access_index(
                                      &((typeof((skc)))((skc)))->skc_v6_rcv_saddr.in6_u.u6_addr32));
            break;
        default:
            return false;
    }
...
static void __always_inline parse_sock_key_from_ipv6_tcp_hdr(struct sock_key *key, struct ipv6hdr *ipv6,
                                                             struct tcphdr *tcp)
{
    bpf_probe_read_kernel((void *)(key->sip), 16,
                          (const void *)__builtin_preserve_access_index(&((typeof((ipv6)))((ipv6)))->saddr));
    bpf_probe_read_kernel((void *)(key->dip), 16,
                          (const void *)__builtin_preserve_access_index(&((typeof((ipv6)))((ipv6)))->daddr));
    u16 sport = 0;
    u16 dport = 0;
    BPF_CORE_READ_INTO(&sport, tcp, source);
    BPF_CORE_READ_INTO(&dport, tcp, dest);
...
static __always_inline int parse_skb(void *ctx, struct sk_buff *skb, bool sk_not_ready, enum step_t step)
{
    ...
    if (is_l2) {
        if (network_header && mac_header >= network_header) {
             ...
             */
            // bpf_printk(" mac_header >= network_header: %s, ifindex: %d", _C(skb,dev)->name,
            // _C(skb,dev,ifindex));//629
...
SEC("xdp")
int xdp_proxy(struct xdp_md *ctx) { return XDP_PASS; }

static __always_inline int handle_skb_data_copy(void *ctx, struct sk_buff *skb, int offset, struct iov_iter *to,
                                                int len)
{
    struct sock_key key = { 0 };
...
int BPF_KPROBE(ip_queue_xmit2, struct sk_buff *skb) { return handle_ip_queue_xmit(ctx, skb); }

SEC("kprobe/__ip_queue_xmit")
int BPF_KPROBE(ip_queue_xmit, void *sk, struct sk_buff *skb) { return handle_ip_queue_xmit(ctx, skb); }
...
static __inline void read_sockaddr_kernel(struct conn_info_t *conn_info, const struct socket *socket)
{
...
static __always_inline bool create_conn_info(void *ctx, struct conn_info_t *conn_info, uint64_t tgid_fd,
                                             const struct sock_key *key, enum endpoint_role_t role, uint64_t start_ts)
{
...
static __always_inline void submit_new_conn(void *ctx, uint32_t tgid, int32_t fd, const struct sockaddr *addr,
                                            const struct socket *socket, enum endpoint_role_t role, uint64_t start_ts)
{
...
static __always_inline bool create_conn_info_in_data_syscall(void *ctx, struct tcp_sock *tcp_sk, uint64_t tgid_fd,
                                                             enum traffic_direction_t direct, ssize_t bytes_count,
...
static __always_inline void process_syscall_data_vecs(void *ctx, struct data_args *args, uint64_t id,
                                                      enum traffic_direction_t direct, ssize_t bytes_count,
                                                      bool is_in_nested_ssl)
{
    ...
    if (!conn_info) {
        ...
        if (tcp_sk) {
            ...
            if (new_conn_info) {
                ...
                bool created = create_conn_info_in_data_syscall(ctx, tcp_sk, tgid_fd, direct, bytes_count,
                                                                new_conn_info, args->end_ts - 1);
                if (created) {
...
#pragma unroll
            for (size_t i = 0; i < PROTOCOL_VEC_LIMIT && i < args->iovlen; i++) {
                ...
                if (buf_size != 0) {
                    ...
                    struct protocol_message_t protocol_message
                        = infer_protocol(iov_cpy.iov_base, buf_size, bytes_count, conn_info, trace_protocol);
        ...
        conn_id_s.tgid_fd = tgid_fd;
        enum step_t step = direct == kEgress ? SYSCALL_OUT : SYSCALL_IN;
        report_syscall_buf_without_data(ctx, seq, &conn_id_s, bytes_count, step, args->start_ts,
                                        args->end_ts - args->start_ts, args->source_fn);
    }
...
static __always_inline void process_syscall_data(void *ctx, struct data_args *args, uint64_t id,
                                                 enum traffic_direction_t direct, ssize_t bytes_count,
                                                 bool is_nested_ssl)
{
    if (bytes_count <= 0) {
...
static __always_inline void process_syscall_sendfile(void *ctx, uint64_t id, struct sendfile_args *args,
                                                     ssize_t bytes_count)
{
    ...
    if (!conn_info) {
        ...
        if (tcp_sk) {
            ...
            if (new_conn_info) {
                new_conn_info->protocol = kProtocolUnset;
                bool created = create_conn_info_in_data_syscall(ctx, tcp_sk, tgid_fd, kEgress, bytes_count,
                                                                new_conn_info, args->end_ts - 1);
                if (created) {
                    conn_info = bpf_map_lookup_elem(&conn_info_map, &tgid_fd);
                }
...
static __always_inline void process_implicit_conn(void *ctx, uint64_t id, const struct connect_args *args,
                                                  enum source_function_t source_fn, enum endpoint_role_t role)
{
    uint32_t tgid = id >> 32;
...
static __always_inline bool update_fd_and_syscall_len_to_uprobe(struct nested_syscall_fd_t *nested_syscall_fd_ptr,
                                                                int fd, int len)
{
...
int BPF_PROG(fentry_security_socket_sendmsg) { return handle_security_socket_sendmsg(); }

SEC("kprobe/security_socket_sendmsg")
int BPF_KPROBE(security_socket_sendmsg_enter) { return handle_security_socket_sendmsg(); }
...
int BPF_PROG(fentry_security_socket_recvmsg) { return handle_security_socket_recvmsg(); }

SEC("kprobe/security_socket_recvmsg")
int BPF_KPROBE(security_socket_recvmsg_enter) { return handle_security_socket_recvmsg(); }
...
#endif
int BPF_PROG(fexit__sys_recvfrom, struct pt_regs *regs, ssize_t ret) { return handle_syscall_exit_recvfrom(ctx, ret); }
...
#endif
int BPF_PROG(fexit__sys_read, struct pt_regs *regs, long int ret) { return handle_syscall_exit_read(ctx, ret); }
...
#endif
int BPF_PROG(fexit__sys_recvmmsg, struct pt_regs *regs, long int ret) { return handle_syscall_exit_recvmmsg(ctx, ret); }
...
#endif
int BPF_PROG(fexit__sys_recvmsg, struct pt_regs *regs, long int ret) { return handle_syscall_exit_recvmsg(ctx, ret); }
...
#endif
int BPF_PROG(fexit__sys_readv, struct pt_regs *regs, long int ret) { return handle_syscall_exit_readv(ctx, ret); }
...
int BPF_PROG(fentry__sys_sendto, int fd, void *buff, size_t len, unsigned int flags, struct sockaddr *dest_addr,
             int addrlen)
{
...
#endif
int BPF_PROG(fexit__sys_sendto, struct pt_regs *regs, ssize_t ret) { return handle_syscall_exit_sendto(ctx, ret); }
...
int BPF_PROG(fentry__sys_sendmsg, int fd, struct my_user_msghdr *msg) { return handle_syscall_enter_sendmsg(fd, msg); }
...
#endif
int BPF_PROG(fexit__sys_close, struct pt_regs *regs, long int ret) { return handle_syscall_exit_close(ctx, ret); }
...
int BPF_PROG(fentry__sys_accept4, int fd, struct sockaddr *addr) { return handle_syscall_enter_accept4(addr); }
...
int BPF_PROG(fexit__sys_accept4, int fd, struct sockaddr *upeer_sockaddr, int *upeer_addrlen, int flags, long int ret)
{
