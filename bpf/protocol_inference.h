...
static __always_inline enum message_type_t infer_kafka_request(const char *buf)
{
    ...
    if (request_API_version < 0 || request_API_version > kMaxAPIVersion) {
        // bpf_printk("kafka request API version:%d, kMaxAPIVersion:%d, apikey:%d", request_API_version, kMaxAPIVersion,
        // request_API_key);
        return kUnknown;
    }
...
static __always_inline bool is_kafka_protocol(const char *buf, size_t count, size_t total_count,
...
static __inline enum message_type_t is_mongo_protocol(const char *old_buf, size_t count)
{
    ...
    if (opcode == kOPUpdate || opcode == kOPInsert || opcode == kReserved || opcode == kOPQuery || opcode == kOPGetMore
        || opcode == kOPDelete || opcode == kOPKillCursors || opcode == kOPCompressed || opcode == kOPMsg) {
        if (response_to == 0) {
            return kRequest;
        }
    }
...
static __always_inline enum message_type_t is_rocketmq_protocol(const char *old_buf, size_t count)
{
    ...
    if (serialized_type == 0x0) { // json format
        ...
        if (buf[0] != '{' || buf[1] != '"' || buf[2] != 'c' || buf[3] != 'o' || buf[4] != 'd' || buf[5] != 'e'
            || buf[6] != '"' || buf[7] != ':') {
            // {"code":
            return kUnknown;
        }
...
static __always_inline struct protocol_message_t infer_protocol(const char *buf, size_t count, size_t total_count,
                                                                struct conn_info_t *conn_info,
                                                                enum traffic_protocol_t trace_protocol)
{
    ...
    } else if (TRACE_PROTOCOL(kProtocolMySQL)
               && (protocol_message.type = is_mysql_protocol(buf, count, conn_info)) != kUnknown) {
        protocol_message.protocol = kProtocolMySQL;
    } else if (TRACE_PROTOCOL(kProtocolRocketMQ)
               && (protocol_message.type = is_rocketmq_protocol(buf, count)) != kUnknown) {
        protocol_message.protocol = kProtocolRocketMQ;
    } else if (TRACE_PROTOCOL(kProtocolKafka)
               && (protocol_message.type = is_kafka_protocol(buf, count, total_count, conn_info)) != kUnknown) {
