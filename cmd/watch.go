...
func init() {
	...
	watchCmd.PersistentFlags().BoolVar(&options.WatchOptions.TraceSocketEvent, "trace-socket-event", false, "收集套接字层事件，以测量套接字数据传输所花费的时间。")
	watchCmd.PersistentFlags().BoolVar(&options.WatchOptions.TraceSslEvent, "trace-ssl-event", true, "收集 SSL 事件以追踪 SSL 连接数据。")
